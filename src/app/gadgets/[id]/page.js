...
async function getData(id) {
    const res = await fetch(`https://trademovers.onrender.com/products/productCategory/gadgets/${id}`, { cache: 'no-store' });
    if (!res.ok) {
...
const Page = async ({params}) => {
    ...
    return (
            ...
            <div className={styles.container}>
                <h2 className={styles.movie_title}>   gadgets \ <span> {data.productname} </span> </h2>
